...
def test_explicit_awq_override_wins_on_cpu() -> None:
    config = InferenceConfig(quantization=QuantizationMode.AWQ)
...
def test_disabling_offload_removes_host_memory_flags() -> None:
    config = InferenceConfig(offload={"enabled": False})
...
def test_cuda_plan_fails_when_host_cannot_absorb_vram_shortfall() -> None:
    config = InferenceConfig(offload={"reserve_system_memory_gb": 3.5})
    hardware = DeviceInventory(
        device="cuda",
        cuda_device_count=1,
        system_memory_gb=4,
...
def test_cpu_plan_fails_early_when_quantized_model_exceeds_ram() -> None:
    config = InferenceConfig(offload={"reserve_system_memory_gb": 2})
    hardware = DeviceInventory(
        device="cpu",
        system_memory_gb=5,
        detection_source="test",
